...
class ZolaAI:
    ...
    def order_moves(self, moves, tt_best_move=None):
        """
        Move Ordering con priorità assoluta alla mossa migliore trovata dalla Transposition Table.
        """
        def move_score(move):
            # Se è la mossa migliore salvata dalla Transposition Table, controllala per PRIMA in assoluto
            ...
            dest_dist = self.game.get_distance_level(tr, tc)
            score -= dest_dist 
    ...
    def search(self, state):
        ...
        # Svuotiamo la cache per liberare memoria (o la manteniamo limitata per evitare memory leak)
        # In tornei brevi, pulirla a ogni nuova mossa è più sicuro e fa risparmiare memoria
        ...
        print(f"[AI Supremo {self.root_player}] Profondità raggiunta: {depth-1}. Mossa scelta: {best_move}")
