...
class ZolaAI:
    ...
    def evaluate_state(self, state):
        ...
        utilizzando la stessa logica di aggressivePlayer.
        """
    ...
    def order_moves(self, moves):
        """
        Move Ordering per massimizzare l'efficienza dell'Alpha-Beta Pruning.
        Ordina le mosse dando priorità assoluta alle catture.
        """
        def move_score(move):
            # Formato in ZolaGameS: move = ((fr, fc), (tr, tc), is_capture)
            fr, fc = move[0]
            tr, tc = move[1]
            is_capture = move[2]

            score = 0
            # Le catture sono sempre le mosse più importanti da valutare per prime
            if is_capture:
                score += 1000

            # A parità di cattura (o non cattura), preferiamo muoverci verso il centro
            # Un distance level più basso significa più vicino al centro
